...
def notify_document_deleted(sender, instance, **kwargs):
    ...
    if instance.owner and instance.owner.email:
        ...
        connection = get_connection(ssl_context=context)

        email = EmailMessage(
            'Documento eliminado',
            f'Se ha eliminado el documento: {instance.title}',
            'paperlesssoporte@gmail.com',
            [instance.owner.email],
            connection=connection,
        )
        try:
            email.send(fail_silently=False)
            print(f"Correo enviado a {instance.owner.email} correctamente.")
        except Exception as e:
            print(f"Error al enviar correo a {instance.owner.email}: {e}")
